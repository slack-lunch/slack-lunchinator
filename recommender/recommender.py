...
class Recommender:
    ...
    def process_meal_name(self, meal):
        words = re.findall(self.REGEX_WORDS, meal)
        words_clean = set()

        for word in words:
            mf = self.morph.find(word)
            if len(mf) > 0 and 'lemma' in mf[0]:
                word_lemma = mf[0]['lemma']
                if word_lemma.lower() not in self.stopwords:
    ...
    def transform_to_dataset(self, meals, words_indecies, restaurants_indecies, label=None):
        ...
        for i, m in enumerate(meals):
            ...
            X[i, r_idx] = 1
            for w in self.process_meal_name(m.name):
    ...
    def get_recommendations(self, n=10):
        ...
        words = list(set((w for m in Meal.objects.all() for w in self.process_meal_name(m.name))))
        ...
        selected_meals_df = self.transform_to_dataset(selected_meals, words_indecies, restaurants_indecies, 1)
        not_selected_meals_df = self.transform_to_dataset(not_selected_meals, words_indecies, restaurants_indecies, 0)
        train_meals_df = np.concatenate((selected_meals_df, not_selected_meals_df))

        todays_meals = Meal.objects.filter(date=date.today())
        todays_meals_df = self.transform_to_dataset(todays_meals, words_indecies, restaurants_indecies)

        X, y = train_meals_df[:, :-1], train_meals_df[:, -1]
        clf = self.get_trained_model(X, y)

        predictions = self.get_predictions(todays_meals_df, clf)
